 ...
 */
package com.selene.volley.image;
...
abstract public class ImageLoader {
	 ...
	 * onSuccess(response, true) will be invoked to reflect any cached data that
	 * was already available. If the data was available, response.getBitmap()
	 * will be non-null.
	 *
	 * 2. After a network response returns, only one of the following cases will
	 * happen: - onSuccess(response, false) will be called if the image was
	 * loaded. or - onError will be called if there was an error loading the
	 * image.
	 */
	 ...
	 * {@link #get(String, ImageListener, int, int, ScaleType)} with
	...
	public ImageContainer get(String requestUrl, ImageListener imageListener, int maxWidth, int maxHeight, ScaleType scaleType) {
		...
		// network and track it.
		ImageRequest newRequest = buildRequest(requestUrl, maxWidth, maxHeight, scaleType, Config.RGB_565);
	...
	abstract public ImageRequest buildRequest(String requestUrl, int maxWidth, int maxHeight, ScaleType scaleType, Config config);

	// protected Request<Bitmap> makeImageRequest(String requestUrl, int
	// maxWidth, int maxHeight, ScaleType scaleType, final String cacheKey) {
	// return new ImageRequest(Method.GET, requestUrl, new Listener<Bitmap>() {
	// @Override
